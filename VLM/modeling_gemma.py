...
# convert image features dim to combined embedding dim
class PaliGemmaMultimodalProjector(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.linear_layer =nn.Linear(config.vision_config.hidden_size, config.vision_config.projection_dim, bias=True)
    def forward(self, image_features):
        hidden_state = self.linear_layer(image_features)
        return hidden_state
...
# language model
class PaliGemmaCausalLM(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def forward(self, attention_mask, position_ids, input_embed, kv_cache):
        outputs = self.model(
            attention_mask=attention_mask,
            position_ids=position_ids,
...
class PaliGemmaforConditionalGeneration(nn.Module):
    def __init__(self, config : PaliGemmaConfig):
        ...
        language_model = PaliGemmaCausalLM(config.text_config)
    ...
    def _merge_text_and_image_tokens(self,
        image_features, input_embed, input_ids, attention_mask, kv_cache=None
):
        _, _, embed_dim = image_features.shape
        batch_size, seq_len = input_ids.shape
        dtype , device = input_embed.dtype, input_embed.device
        # b, seq_len, hidden_size
        scaled_image_features = image_features / math.sqrt(self.config.hidden_size**0.5)
        
        final_embedding = torch.zeros((batch_size, seq_len,embed_dim), dtype=input_embed.dtype,device=input_embed.device)
        # pick the text embedding from the final_embeddig
        text_mask = (input_ids !=self.config.image_token_index) & (input_ids!=self.config.pad_token_id)
        # pick the image embedding from the final_embedding
        image_mask = (input_ids == self.config.image_token_index)
        # pick the padded token
        ...
        # copy the text embedding to the final embedding
        final_embedding = torch.where(text_mask, input_embed, final_embedding)
        # copy the image embedding to the final embedding
        final_embedding = final_embedding.masked_scatter(image_mask,scaled_image_features)
        # zero out the pad tokens
        final_embedding = torch.where(pad_mask, torch.zeros_like(final_embedding), final_embedding)

        # create attention mask
        dtype , device = input_embed.dtype, input_embed.device
        min_dtype = torch.finfo(dtype).min
        q_len = input_embed.shape[1]

        # if kv cache is empty, prefilling phase, no need to mask
        if kv_cache is None or kv_cache.num_items()==0:
            causal_mask = torch.full((batch_size, q_len, q_len), fill_value=0, dtype=dtype, device=device)
        else:
            assert q_len==1
            kv_cache_len = kv_cache.num_items() + q_len
            causal_mask = torch.full((batch_size, q_len, kv_cache_len), fill_value=0,dtype=dtype,
                                     device=device)
            
            # add causal mask to every head
            # b, q_len, kv_cache_len -> b, n_heads, q_len, kv_cache_len
        causal_mask = causal_mask.unsqueeze(1)

        # positional info for rope
        if kv_cache is None or kv_cache.num_items()==0:
    ...
    def forward(self,input_ids, pixel_values, attention_mask,kv_cache=None):
        assert torch.all(attention_mask==1), "no mask"
        # currently just a placeholder since the image tokens are all <image>
        ...
        # merge image tokens and text tokens
        input_embed, attention_mask, position_ids = self._merge_text_and_image_tokens(
            image_features, input_embed, input_ids, attention_mask, kv_cache
        )

        return self.language_model(attention_mask=attention_mask, position_ids=position_ids, 
                                   input_embed=input_embed, kv_cache=kv_cache)
