...
def build_or_get_tokenizer(config, ds, lang):
...
def get_ds(config):
    ...
    tokenizer_src = build_or_get_tokenizer(config, ds_raw, config['lang_src'])
    tokenizer_tgt = build_or_get_tokenizer(config, ds_raw, config['lang_tgt'])
    ...
    # choosing max src sequence length and tgt sequence length
    max_len_src = 0
    max_len_tgt = 0
    ...
    print("max src length is : {} and max tgt length is : {}".format(max_len_src, max_len_tgt))
...
def train_model(config):
    ...
    initial_epoch = 0
    global_step = 0
    # if we have a trained model saved to file
    if config['preload']:
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # prediction : [batch_size * seq_len, target vocab_size] labels : [batch_size * seq_len]
